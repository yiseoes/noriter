...
| **자율 디버깅 루프** | QA 실패 → CTO 버그 분류 → 해당 에이전트 수정 → 재검증 (~~최대 3회~~ **최대 1회**, 비용 최적화 목적) |
...
│  ├─ JsRuntimeValidator (Node.js 런타임 실행 검증)             │
...
└───────────────┬───────────────┬───────────────┬───────────────┘
             ...
             MySQL          Claude API          FileSystem
                         (Sonnet 4.6:         /workspace/
                          Frontend/Backend/QA  {projectId}/
                          Haiku 4.5:
                          Planning/Content
                          /CTO/Design)
```
...
│ 정적 코드 분석 (SYNTAX / STRUCTURE / LOGIC /                │
│               GAME_STRUCTURE / KOREAN_IME 등)               │
│ PASS / FAIL 판정 (CRITICAL/HIGH 버그 없으면 PASS)           │
└──────────────┬──────────────────────┬────────────────────────┘
               │ PASS                 │ FAIL
               ▼                      ▼
┌─────────────────┐    ┌──────────────────────────────────────┐
│ STAGE 7: RELEASE│    │ STAGE 6: DEBUG  (~~최대 3회~~ 최대 1회) │
│                 │    │                                       │
    ...
    "gameClass": {
      "publicMethods": [
        ...
        { "name": "getGameState", "signature": "getGameState(): object",
          "description": "현재 상태 plain object 반환 — null/undefined 금지" },
...
> **계약 변경 이력**: ~~`setGame(game)` 메서드~~ 제거 — `render(game)`이 game 객체를 직접 받는 방식으로 통일. `getGameState()` 명시 추가 — 누락 시 QA가 HIGH로 FAIL 처리하기 때문.
...
| **AI** | Claude API (Anthropic) | Sonnet 4.6 / Haiku 4.5 혼합 | 품질 필요 단계(Frontend/Backend/QA)는 Sonnet, 나머지는 Haiku (비용 최적화) |
...
│     progress   INT (0~100)       │
│     debugAttempts INT            │
│     maxDebugAttempts INT         │  ← ~~기본값: 3~~ → 기본값: 1 (비용 최적화)
...
- GAME_STRUCTURE — 아키텍처 계약 위반 (이중 루프, 이벤트 리스너 중복, Renderer constructor, ctx hygiene)
...
| **이중 게임 루프** (게임 2배속 + 크래시) | `cto-debug-system.txt`가 "rAF를 Game 생성자에 추가"를 지시 — 디버그 사이클마다 이중 루프 재생성 | 잘못된 지시 제거 + 전 에이전트에 GAME LOOP 금지 규칙 추가 |
| **Renderer constructor TypeError** (`canvas.getContext is not a function`) | AI가 `constructor(canvas)` 생성 → `new Renderer()` 호출 시 canvas=undefined | CONSTRUCTOR RULE 추가 (`constructor()` 무조건 인자 없음) + QA GAME_STRUCTURE 체크 |
| **게임 화면 우측 쏠림** (플레이어가 화면 밖으로 이탈) | Game이 `canvas.width`(실제 픽셀)를 사용, Renderer는 800 기준으로 스케일 → 좌표 2~3배 오차 | 800×600 논리 좌표계 계약 전 에이전트에 명시, scaleX/Y 매 프레임 갱신 규칙 |
| **피드백/디버그 CTO가 CSS·Renderer 못 봄** | `cto-feedback-user.txt`, `cto-debug-user.txt` 변수 누락 | user 템플릿에 `{{styleCss}}`, `{{renderCode}}` 추가 + `CtoAgent.java` Map.of() 동기화 |
| **ctx 누적 transform** (매 프레임 그림 어긋남) | `ctx.save()` 없이 `ctx.scale()` 사용 → 이전 프레임 transform 유지됨 | CTX STATE HYGIENE 규칙 + `ctx.globalAlpha` 리셋, `save/restore` 균형 검증 QA 추가 |
...
class Renderer {
  constructor() {
    this.canvas = null;   // ← 생성자에 인자 없음 (canvas는 init으로 주입)
    this.ctx = null;
    this.scaleX = 1;
    this.scaleY = 1;
  }
  init(canvas) {
    ...
    // ← scaleX/Y는 여기서 계산 금지 (iframe 내 offsetWidth=0)
  }
  render(game) {
    // 매 프레임 scale 갱신 (iframe 내에서 offsetWidth가 첫 프레임 이후 확정됨)
    this.scaleX = (this.canvas.offsetWidth || this.canvas.width || 800) / 800;
    this.scaleY = (this.canvas.offsetHeight || this.canvas.height || 600) / 600;
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    // game.state, game.score 등 game 객체 직접 접근
    // game.getGameState() 호출 금지
  }
}
...
window.addEventListener('load', () => {
  ...
  // 마우스/터치 이벤트: 800×600 논리 좌표로 스케일링
  canvas.addEventListener('mousemove', (e) => {
    const rect = canvas.getBoundingClientRect();
    const x = (e.clientX - rect.left) * (800 / rect.width);
    const y = (e.clientY - rect.top) * (600 / rect.height);
    if (game.handleMouseMove) game.handleMouseMove(x, y);
  });
  ...
  window.addEventListener('resize', () => { renderer.init(canvas); });
...
### ⑥ 이중 게임 루프 — 디버그할수록 악화되는 버그

**증상**: 게임이 2배속으로 실행되고, 일정 시간 후 크래시
**원인**: `back-main-system.txt`는 "Game 내부에 rAF 금지"라고 했지만, `cto-debug-system.txt`는 QA FAIL 시 "Game 생성자에 `requestAnimationFrame` 추가하라"고 지시하고 있었음. 두 프롬프트가 정반대 지시를 내려 디버그 사이클이 돌 때마다 이중 루프가 재생성됨.

```
[외부 initializationCode] → requestAnimationFrame(gameLoop)  ← 정상
[Game 생성자 내부]         → requestAnimationFrame(this.gameLoop)  ← 버그 재생성
→ 게임 루프 2개 병렬 실행 → 매 프레임 update() 2번 호출 → 2배속 + 충돌
```

**해결**: `cto-debug-system.txt`의 잘못된 지시 제거. 모든 에이전트 프롬프트에 "GAME LOOP — rAF는 외부 init 코드 전용, Game 내부 금지" 규칙을 일관 적용.

### ⑦ Renderer constructor TypeError

**증상**: 게임 로드 직후 `Uncaught TypeError: Cannot read properties of undefined (reading 'getContext')` 발생
**원인**: AI가 `constructor(canvas)` 형태로 Renderer를 생성 → 외부 init 코드는 `new Renderer()`(인자 없음)로 호출 → `this.canvas = undefined` → `getContext()` 즉시 TypeError

```javascript
// AI가 생성한 잘못된 코드
class Renderer {
  constructor(canvas) {               // ← 문제: 인자 있음
    this.ctx = canvas.getContext('2d');  // ← canvas=undefined → TypeError
  }
}

// 올바른 패턴
class Renderer {
  constructor() { this.canvas = null; }  // ← 인자 없음
  init(canvas) { this.canvas = canvas; this.ctx = canvas.getContext('2d'); }
}
```

**해결**: `front-main-system.txt`에 CONSTRUCTOR RULE 추가 (`constructor()` 인자 없음 강제), QA GAME_STRUCTURE 체크에 포함.

### ⑧ 800×600 논리 좌표 vs 실제 캔버스 크기 충돌

**증상**: 플레이어가 화면 오른쪽으로 쏠리거나 화면 밖으로 이탈. 좌클릭 위치가 실제 위치와 2~3배 차이.
**원인**: Game 로직이 `canvas.width`(실제 px, 예: 1920)를 사용하는데, Renderer는 800 기준 스케일로 그림을 그림 → 게임 객체 위치 × scaleX = 실제 렌더 위치가 화면 밖으로 나감.

```javascript
// 잘못된 예: canvas.width는 실제 픽셀 (1920px 화면에서 1920)
this.BOARD_WIDTH = canvas.width;  // ← 1920, Renderer는 800 기준

// 올바른 예: 논리 좌표 고정
this.LOGICAL_WIDTH = 800;
this.LOGICAL_HEIGHT = 600;
// Renderer가 내부적으로 scaleX = actualWidth / 800 적용
```

**해결**: 전 에이전트 프롬프트에 "800×600 논리 좌표계 계약" 추가. 마우스 이벤트도 init 코드에서 pre-scale: `x = (e.clientX - rect.left) * (800 / rect.width)`.
...
│       └── util/              # IdGenerator, JsSyntaxValidator, JsRuntimeValidator
│   └── src/main/resources/
│       ├── templates/         # 24개 AI 프롬프트 파일 (생성/수정/디버그/피드백 전 경로)
│       ├── game-validator.js  # Node.js 런타임 검증 스크립트
│       ├── application.yml
│       └── log4j2.xml
│
├── frontend/
...
| 에이전트 간 계약 유지 | CTO 아키텍처 JSON으로 인터페이스 명세화, 전 파이프라인 프롬프트 계약 정합성 감사 |
...
| 비용 절감 | 실패 단계만 재실행, ~~디버깅 최대 3회~~ 1회 제한, Haiku/Sonnet 역할별 선택, 토큰 추적 |
| 800×600 좌표계 계약 | Game은 논리 좌표 고정, Renderer가 scaleX/Y로 실제 화면에 투영 — 전 에이전트 프롬프트 일관 적용 |
| 디버그 루프의 역설 (고칠수록 버그) | fix/debug 에이전트에 동일한 아키텍처 금지 규칙 적용 — 수정 사이클이 버그를 재생성하지 않도록 |
...
| DEBUG (발생 시) | 해당 에이전트 | ~20초 | ~~최대 3회~~ → 최대 1회 |
...
- 모델: ~~claude-3-5-haiku-20241022 단일 사용~~ → 역할별 혼합 사용
  - **Sonnet 4.6**: FrontendAgent, BackendAgent, QaAgent (코드 품질 중요)
  - **Haiku 4.5**: PlanningAgent, ContentAgent, CtoAgent, DesignAgent (비용 절감)
- 게임 1개 생성 비용: **약 $0.03 ~ $0.08** (혼합 모델 기준)
...
2. 모델 선택
   - ~~claude-3-5-haiku 단일 사용~~ → 역할별 최적 모델 선택
   - 코드 생성(Frontend/Backend/QA): **Sonnet 4.6** — 구조적 코드 품질 중요
   - 기획/설계/콘텐츠: **Haiku 4.5** — 빠른 응답, JSON 출력으로 충분

3. 디버깅 횟수 제한
   - ~~최대 3회~~ → **최대 1회** 디버깅 루프 (과도한 API 비용 방지)
...
- [x] **프롬프트 종합 감사 + 계약 정합성 수립** (CRIT 4건, HIGH 7건, MEDIUM 5건 전부 해결)
  - 이중 게임 루프, Renderer constructor, 좌표계, ctx hygiene 등 반복 발생 버그 근절
  - fix/debug/feedback 파이프라인 전 경로에 동일한 아키텍처 계약 적용
